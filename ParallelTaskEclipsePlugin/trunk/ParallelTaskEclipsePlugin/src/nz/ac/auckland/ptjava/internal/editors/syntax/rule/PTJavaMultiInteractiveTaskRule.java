...
/**
 * A custom rule for highlighting rules that look similar to <code>"IO_TASK(4)"</code>
 * or <code>"IO_TASK(*)"</code> or <code>"IO_TASK(field)"</code>.
 *
 */
public class PTJavaMultiInteractiveTaskRule implements IRule {

	private static final String fTaskString = "IO_TASK(";
	 ...
	 */
	public PTJavaMultiInteractiveTaskRule(IToken successToken) {
	...
	public IToken evaluate(ICharacterScanner scanner) {
		...
		int count = 0;
		StringBuffer s = new StringBuffer(17);
		while (true) {
			//  read in a char
			int r = scanner.read();
			count++;
		...
		//PTJavaLog.logInfo("s.size(): " + s.length() ); 
