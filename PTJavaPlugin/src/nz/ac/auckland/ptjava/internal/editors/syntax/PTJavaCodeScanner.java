...
/**
 * The scanner for the java code (The "default" document partition).
 * Current version is very similar to JavaCodeScanner, augmented with
 * extra PTJava keywords.
 * 
 */
//  The "old" version that does not rely on Abstract Java Scanner is located
//  in revision 805 of the taschto/nz.ac.auckland.ptjava svn repository.
public class PTJavaCodeScanner extends AbstractJavaScanner {
	 ...
	 */
	public PTJavaCodeScanner(IColorManager manager, IPreferenceStore store) {
		super(manager, store);
		initialize();
	}
	...
	protected List createRules() {
		...
		//  add custom PTJava multi task rule (colored in Java colors)
		token= getToken(IJavaColorConstants.JAVA_KEYWORD);
		rules.add(new PTJavaMultiTaskRule(token));
		
		//  add custom PTJava multi interactive task rule (colored in Java colors)
		token= getToken(IJavaColorConstants.JAVA_KEYWORD);
		rules.add(new PTJavaMultiInteractiveTaskRule(token));
		...
		//  add other PTJava rules
		token= getToken(IJavaColorConstants.JAVA_KEYWORD);
		for (int i= 0; i < fgPTJavaKeywords.length; i++)
			wordRule.addWord(fgPTJavaKeywords[i], token);
